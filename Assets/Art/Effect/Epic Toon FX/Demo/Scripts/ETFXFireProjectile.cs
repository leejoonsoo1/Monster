...
namespace EpicToonFX
{
    public class ETFXFireProjectile : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                if (false == EventSystem.current.IsPointerOverGameObject()) //Checks if the mouse is not over a UI part
                {
                    if (true == Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit, 100f)) //Finds the point where you click with the mouse
                    {
                        GameObject projectile = Instantiate(projectiles[currentProjectile], 
